...
{
    public partial class FRM_Order_Add : MetroFramework.Forms.MetroForm
    {
        double QtyinStack = 0;
        ...
        private void btnNew_Click(object sender, EventArgs e)
        {
            ...
            errorProvider1.SetError(txtProductCount, null);

        }

        private void FRM_Order_Add_Load(object sender, EventArgs e)
        {
        ...
        private void txtProductCount_KeyUp(object sender, KeyEventArgs e)
        {
            try
            {
                if (double.Parse(txtProductCount.Text) > QtyinStack)
                {
                    errorProvider1.SetError(txtProductCount, string.Format("this County larger than in stack ({0})", QtyinStack));
                    errorProvider1.SetIconPadding(txtProductCount, -20);
                ...
                else
                {
                    ...
                    double saleTotal = double.Parse(txtSalePrice.Text) * double.Parse(txtProductCount.Text);
                    txtProductTotal.Text = saleTotal.ToString();
                    //*****************************************
                    double Discount;
                    double.TryParse(txtProductDiscount.Text, out Discount);
                    double buyDiscount = (double.Parse(txtProductTotal.Text) * Discount) / 100;
                    double buyFinal = saleTotal - buyDiscount;
        ...
        void calculating()
        {
            ...
            {
                totalTotals += Convert.ToDouble(dataGridView1.Rows[i].Cells[4].Value);
                totalDiscount += Convert.ToDouble(dataGridView1.Rows[i].Cells[4].Value) * Convert.ToDouble(dataGridView1.Rows[i].Cells[5].Value) / 100;
                totalFinals += Convert.ToDouble(dataGridView1.Rows[i].Cells[6].Value);
            }
        ...
        private void btnEditDataDet_Click(object sender, EventArgs e)
        {
            if (dataGridView1.SelectedRows.Count > 0)
            {
                ...
                txtSalePrice.Text = dataGridView1.CurrentRow.Cells["Column3"].Value.ToString();
        ...
        private void btnAddDataDet_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                dataGridView1.Rows.Add(txtProductId.Text, cmbProductName.Text, txtSalePrice.Text, txtProductCount.Text,
                    txtProductTotal.Text, discount.ToString(), txtProductAmount.Text, txtProductNote.Text);
                //***********************************************
                txtProductId.Text = cmbProductName.Text = cmbBarcode.Text =
                txtSalePrice.Text = txtProductTotal.Text = txtProductAmount.Text =
                txtProductNote.Text = "";
                txtProductDiscount.Text = txtProductCount.Text = "0";
                calculating();
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            {
                Sales.Sales_Insert(DateTime.Now.ToString(), int.Parse(cmbCustomer.SelectedValue.ToString()), txtNote.Text, txtUser.Text, int.Parse(txtCount.Text),
                    int.Parse(txtdiscount.Text), txtTotalPaid.Text, txtTotalResidul.Text, txtAmount.Text, txtTotal.Text);

                for (int i = 0; i < dataGridView1.Rows.Count; i++)
                {
                    Sales_Details.Sales_Details_Insert(int.Parse(txtInvoiceNum.Text), int.Parse(dataGridView1.Rows[i].Cells["Column1"].Value.ToString()),
                        ...
                        dataGridView1.Rows[i].Cells["Column8"].Value.ToString());
                }
                ...
                BL.Logs.Logs_Add(Program.UserFullName, DateTime.Now.ToString(), "Add  Order Invoice");
            }
        }
    }
}
